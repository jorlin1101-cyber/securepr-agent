...
SecurePR Agent 是一个面向 Pull Request 的风险审查、安全修复与持续进化平台。
...
- 声明式 Agent Skill `SKILL.md` 工件校验、版本化、回放门禁和回滚
...
$env:SECUREPR_AUTH_REQUIRED = 'true'
$env:SECUREPR_AUTH_SECRET = [Convert]::ToBase64String($bytes)
$env:SECUREPR_BOOTSTRAP_ADMIN_USERNAME = 'admin'
$env:SECUREPR_BOOTSTRAP_ADMIN_PASSWORD = '<替换为至少 10 个字符的密码>'

python -m securepr_agent
```

不要直接使用示例占位符作为密码或密钥。环境变量只对当前 PowerShell 及其子进程生效；修改配置后需要停止并重新启动 SecurePR Agent。服务可以在未配置模型时启动并提供健康检查，但提交审查前必须按下方“模型配置”章节配置模型。
...
```powershell
$env:SECUREPR_GUEST_ACCESS_ENABLED = 'true'
```

访客按钮不会在浏览器中保存或公开管理员凭据。服务端只签发短时效的 `guest` 会话，权限限定为读取演示数据和发起代码审查，不能执行修复、系统管理或审计操作。演进实验室的回放评测需要管理员登录；它可能在候选通过门禁后激活线上提示词，访客只能查看就绪状态与评测记录。生产环境默认关闭该入口；仅应在使用受控样例数据的演示实例中启用。
...
- Agentic 审查必须配置真实模型；没有模型时服务仍可提供健康检查，但不会把规则扫描伪装成 LLM Agent 结果。
- 当前项目用于作品集演示和工程验证，自动生成的审查结果在合并前仍需人工复核。
...
项目启动时会自动读取项目根目录的 `.env`，也兼容 `securepr_agent/.env`；系统环境变量优先于 `.env` 文件。推荐将以下内容写入根目录 `.env`（该文件已被 `.gitignore` 忽略）：

```env
SECUREPR_LLM_PROVIDER=deepseek
SECUREPR_DEEPSEEK_API_KEY=你的真实APIKey
```
...
$env:SECUREPR_GITHUB_WEBHOOK_SECRET = [Convert]::ToBase64String($webhookBytes)

# 私有仓库、PR 评论回写或自动修复需要；只审查公开仓库且不回写时可以不配置。
$env:SECUREPR_GITHUB_TOKEN = '<GitHub fine-grained PAT>'
...
Webhook Secret 用于验证 GitHub 请求头中的 HMAC-SHA256 签名，不能与登录用的 `SECUREPR_AUTH_SECRET` 混用。Webhook 请求不携带管理台 Bearer Token；`/webhooks/github` 使用签名而不是用户登录进行认证。
...
只接收 Webhook 但不访问私有仓库、不回写评论且不执行自动修复时，可以不设置 PAT。密钥必须在启动 SecurePR Agent 前设置，修改后需要重启服务。
...
命令启动后会显示一个形如 `https://example.trycloudflare.com` 或 `https://example.ngrok-free.app` 的公网 HTTPS 地址。保持 SecurePR Agent 和转发进程同时运行。临时公网地址通常会在转发工具重启后变化，变化后必须同步更新 GitHub Webhook 的 Payload URL。

上述快捷转发会把 8080 端口上的管理台和 API 一并暴露到公网，因此必须保持 `SECUREPR_AUTH_REQUIRED=true`，并使用强管理员密码和随机 `SECUREPR_AUTH_SECRET`。长期部署建议通过反向代理只公开 `/webhooks/github`（以及按需公开 `/health`），不要向公网暴露整个管理台。
...
- **Secret**：与 `SECUREPR_GITHUB_WEBHOOK_SECRET` 完全相同；
...
SecurePR Agent 会处理 `opened`、`reopened` 和 `synchronize` 三种 PR 动作；其他 `pull_request` 动作会正常接收但被忽略。服务会根据 payload 中的 `diff_url` 下载 Diff，并异步创建审查任务。
...
默认只在管理台保存结果。只有 `SECUREPR_AUTO_POST_REVIEW=true` 时才会向 PR 回写评论。
...
HTTP / GitHub Webhook
 ...
 ReviewService ── TaskStore(SQLite / PostgreSQL)
        │
        ▼
 ReviewHarness (SecurePR Agent Runtime / checkpoint / resume / budget / trace)
        │
        ├── DiffParser

...
def _migrate_legacy_artifact(artifact: dict, expected_name: str) -> dict:
    ...
    for rule in artifact.get("rules") or []:
        ...
        if not match:
            continue
        lines.extend([
            "<!-- securepr_agent:learned:%s:start -->" % rule_id,
            "## %s" % rule_id, "",
            ...
            "<!-- securepr_agent:learned:%s:end -->" % rule_id, "",
        ])
...
class SkillEvolutionEngine:
    ...
    def auto_propose(
        ...
        for case in failures:
            ...
            if not evidence or len(evidence) > 240 or "\n" in evidence or "\r" in evidence:
                continue
            if "securepr_agent:learned:%s:start" % rule_id in content:
                continue
    ...
    def _remove_learned_block(content: str, rule_id: str) -> str:
        pattern = re.compile(
            r"\n*<!-- securepr_agent:learned:%s:start -->.*?"
            r"<!-- securepr_agent:learned:%s:end -->\n*" % (re.escape(rule_id), re.escape(rule_id)),
            re.DOTALL,
        )
        return pattern.sub("\n", content).rstrip() + "\n"

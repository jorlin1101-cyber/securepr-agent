...
class VerifiedPatchFixer:
    ...
    def create_fix_commits(
        ...
        branch = "securepr/fix-pr-%d-%s" % (pull_request, stamp)
        commit = client.create_atomic_commit(
            repository, branch, source_sha, changed,
            "fix: apply verified SecurePR Agent patch for PR #%d" % pull_request,
        )
        draft = client.create_draft_pull_request(
            repository, "fix: verified SecurePR Agent patch for #%d" % pull_request,
